...
export function vrot(v: Point, theta: number): Point {
  const c = Math.cos(theta);
  const s = Math.sin(theta);
  return { x: v.x * c - v.y * s, y: v.x * s + v.y * c };
}

export function vlen(v: Point): number {
  return Math.sqrt(v.x * v.x + v.y * v.y);
}

export function vsetlen(v: Point, len: number): Point {
  return vscale(v, len / vlen(v));
}
